...
class ComputerStrategyEasyTest < Minitest::Test
  ...
  def test_get_move_first_move_no_center_random_chosen
    ...
    computer = ComputerStrategyEasy.new
    assert_includes [0,1,2,3,4,5,6,7,8], computer.get_move(test_board, 'O', 'O', 'X') { |board| false}
  end

  def test_get_move_second_move_no_center_random_chosen
    test_board = ["0", "1", "X", "3", "4", "5", "6", "7", "8"]
    computer = ComputerStrategyEasy.new
    assert_includes [0,1,3,4,5,6,7,8], computer.get_move(test_board, 'O', 'O', 'X') { |board| false}
  end
  ...
  def test_get_move_player_is_ready_to_score_random_chosen
    ...
    computer = ComputerStrategyEasy.new
    engine = GameEngine.new computer
    assert_includes [1,3,5,6,7,8], computer.get_move(test_board, 'O','O', 'X') {|board| engine.game_is_over(board)}
  end
